...
if [ -x "$GOBIN" ]; then
    ...
    if (cd "$ROOT/server/dashboard" && "$GOBIN" build -o /tmp/ft_dashboard . >/tmp/ft_go.log 2>&1); then
        ...
        # 备份下载：合法名可取回内容，路径穿越一律 400
        BAK=$(basename "$D/run"/fwknopd_audit.log.bak-*)
        wget -qO- --header='Authorization: Bearer fttok' \
            "http://127.0.0.1:18099/api/audit/download?bak=$BAK" 2>/dev/null | grep -q '"event":"open"' \
            && ok "审计备份可经面板下载" || bad "审计备份下载"
        wget -qO- --header='Authorization: Bearer fttok' \
