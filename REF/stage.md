...
## 变更日志
- 2026-08-25：**WebUI 2.9.1 归档**（42 项提交，commit `5841246c..` 起）。核心：**一键体检成熟化**——检查项 7→14（新增 fwknopd 二进制 fork 校验/磁盘余量/面板监听地址/管理工具/生效授权存在性/TOFU 悬空绑定/凭证文件权限），异常项钻取闭环（服务控制卡/关于页诊断/预检完整输出+错误行锚定/悬空预填搜索/事件页分流）；**结论可信度**——授权/配置/概览三类拉取失败不再派生误判（统一「无法判定」）、并发两轮只认最新轮、15 个变更点自动降级「建议重检」、超 24h 旧绿降级琥珀、复制报告补时间戳与版本；**后端解析层修复**——明文 HMAC_KEY 写法误判无 HMAC、审计清理与配置备份同秒撞名静默覆盖（防撞序号化+白名单放行）、操作日志导出/「加载更早」补全轮转上一代；**认证语义**——方案导入/导出 401 回登录页附原因、启动日志交代认证形态；**上游二进制错装四表面识别**（体检项/概览红色预警条/CLI 一行命令/概念条目）。回归 115→124 PASS（修复全部挂测）。详见 note/release/2.9.1.md。
- 2026-08-24：**WebUI 2.9.0 归档**（80 项提交，commit `2126a67d..` 起）。核心：**排障定位闭环**——预检失败按错误文件分流直达出错行、保存失败原文模式定位行/结构化模式指令名过滤定位、access.conf 查看器跳行三模式（行号/授权名/#序号）、卡片行号点击直达；**「所搜即所见」高亮收口**——五列表 + 事件展开行 + 配置结构化视图命中高亮、禁用授权区纳入搜索；**配置备份可见可回滚**——备份列表 + 一键回滚（联合预检+现场再备份+热加载）+ 回滚前差异预览（新 bakview 端点双侧掩码），未收录指令说明回退原文注释、指令名补全覆盖全部指令、「防重放收紧」模板；**方案导出/导入迁移闭环**——tar.gz 导出（含明文密钥，文案明示）↔ 导入（成员白名单/大小上限/同名拒覆/不触碰现役配置），方案卡快照摘要可搜；**一键体检**——预检/进程/审计/防火墙/TOTP 监听一致性/占位密钥/时钟漂移 7 项聚合，异常前置 + 直达处置 + 原地重跑 + 纯文本报告复制，首次引导收尾挂体检；**进程级日志面板内出口**——服务日志查看（journalctl→syslog fallback）+ 启动失败根因分流 + 事件空态审计分流；**签发连续作业打磨**——地址重填/选项保留/残留清理、找回路径措辞、批量名单拆行/草稿/二维码、覆盖守卫、提示按服务端实报分态（658 审计定稿）；**撤销/解绑活跃预警**（15 分钟口径单点化）+ 授权卡弱姿态徽标；诊断打包面板别名/异常构成明细、操作日志类别过滤持久化、通知已读水位入 sessionStorage、事件「最近 7 天」时间窗。回归 115/115 PASS。详见 note/release/2.9.0.md。
- 2026-08-24：**WebUI 2.8.0 归档**（100 项提交，commit `c9bce879..` 起）。核心：**凭证分发闭环**——授权 URI 二维码（内嵌 Go 编码器 SVG，签发/重发双入口）、批量签发（名单共享安全选项、实时校验、逐份复制下载、结果直达操作日志核对）、批量签发/撤销/解绑早停三态（会话/权限失败即终止，剩余「未尝试」与失败严格区分）；**审计合规导出**——access.conf 原文查看（密钥掩码+行号/指令高亮）、TOFU/授权/操作日志三路筛选感知 CSV、Content-Length 真实进度、Unicode 文件名清洗；**操作日志深化**——搜索/扩窗 100→300→1000/失败原因入档/类别计数/时间制式/与事件页双向钻取；**登录爆破全链路可见性**——概览警示/导航徽标/标题 ‼/桌面通知节流/诊断打包/按地址消除；**并发防护三连收口**——修复轮询外移架空 mtime 乐观锁（基线锚定+预警条）、stanza 字段 diff 提交、方案应用乐观确认；**事件页正确性**——边界秒多重集差去重（重放风暴不再低估）、事件筛选方案、五列表状态词搜索闭环、防火墙规则结构化视图与验证放行闭环；a11y 第三批（role/aria 语义补齐）；toast 顶出/双重转义/白名单校验/lineDiff 护栏等可靠性修复一批。回归 115/115 PASS。详见 note/release/2.8.0.md。
- 2026-08-24：**WebUI 2.7.0 归档**（39 项提交：37 项功能/修复 + 1 项性能 + 1 项批查文档，commit `04aad70d..` 起）。核心：**管理面可审计性**——面板操作日志 dashboard_ops.jsonl（0600，20 挂点：签发/撤销/解绑/配置保存/服务控制/方案/登录成败/改密，密钥永不入档，关于页日志卡 + 类别/失败过滤 + JSONL 导出，8MB 单代轮转）、登录成功附「上次登录回顾」（上次时间/IP + 此后失败数）；**并发安全**——配置保存 mtime 乐观锁（不再静默覆盖他人修改）+ 授权编辑序号移位身份核验（expect_name/expect_source，均向后兼容）；**配置编辑器成熟化**——合并式落盘保留注释（修 2.3.0 起丢注释缺陷）+ 保存确认内嵌 diff、删除墓碑化（可撤销/过滤可见/保存剔除）、脏标记实比对（改回原值复位）、重建前收割、行级还原；授权编辑器实时改动清单；事件排障知识库动作化（6 类原因建议 + 直达编辑器/配置/legacy 签发动作）；TOFU 设备别名；撤销 TOFU 悬空预警；VPN 长时放行模板；概览 __CHANGEME__ 前置 + 首次运行三步引导；修复 icon() 双重前缀（全部 JS 图标自始未显示）/旧主题变量/密码强度被覆盖/临期警告 once-flag 等一批「自始未生效」缺陷；审计尾读文件偏移增量化（半行不消费）。回归 115/115 PASS（新功能均临时实例端到端冒烟）。详见 note/release/2.7.0.md。
- 2026-08-23：**WebUI 2.6.0 归档**（44 项提交：43 项功能/体验迭代 + 1 项测试基建，commit `cbf53027..` 起）。核心：弱网可靠性加固（fetch 12s/20s 超时、断连指数退避封顶 60s、离线/上线即刻响应、自检失败 5s 倒数自动重试、写操作网络失败统一中文兜底、unhandledrejection 兜底、localStorage 安全助手收口）；授权生命周期闭环——新端点 POST /api/admin/qr + 用户卡「重发 URI」两步弹窗（密钥不离开服务器，URI 参数级掩码）；a11y 第二批（skip link、role=alert、14 处 aria-live、折叠控件 aria 状态、label 关联、SB() 减弱动效、Esc 链接入侧栏与通知角、快捷键标注）；编辑器对称性（授权编辑器实时校验/死配置联动/端口重叠拦截/beforeunload 守卫，签发卡片指纹/TOFU 联动）；配置模板新增 NFQ 与 SPA over HTTP（达 5 枚）；时间呈现对时化（相对↔绝对悬停互换、英文原名 title、展开行强调条、静态刷新时间戳）；主题三态循环；事件搜索 stanza: 前缀；响应缓存头显式化（/api/* no-store）。回归 112→115 PASS（新增 qr 端点 3 用例）。详见 note/release/2.6.0.md。
